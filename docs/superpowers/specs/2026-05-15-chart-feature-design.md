...
| 과금 모델 | 차트 무료 / 분석 대시보드 유료 (월 4,900원 구독, 전 종목 무제한) |
...
유료 엔드포인트. 구독 상태 확인 후 퀀트 상세 데이터 반환.

v1에서는 구독 결제가 미구현이므로, 이 엔드포인트 자체는 구현하되 프론트에서 호출하지 않는다. CTA 버튼 클릭 시 "준비 중" 토스트만 표시.
...
| 차트 분석 구독 | 4,900원/월 | `chart-subscription-monthly` |

`/api/payments/confirm` 엔드포인트에서 orderId prefix로 상품 타입을 구분한다.

**정기결제 주의**: 구독형은 Toss Payments 빌링키(정기결제) API가 필요. v1에서는 CTA 클릭 시 "준비 중입니다" 안내 토스트로 처리하고, 실제 구독 결제 연동은 v2에서 구현한다.

**비용 분석**: 외부 API 비용 $0/요청 (yfinance/ccxt 무료). Railway 서버 $5/월. 구독자 2명이면 손익분기.
