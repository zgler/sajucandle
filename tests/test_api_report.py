...
class TestReportEndpoint:
    def test_report_returns_200(self, client: TestClient):
        with patch("sajucandle.api.routers.saju._has_anthropic_key", return_value=True), \
             patch("sajucandle.api.routers.saju.collect_report_context", return_value={}), \
             patch("sajucandle.api.routers.saju.generate_report",
    ...
    def test_report_no_api_key_returns_503(self, client: TestClient):
        with patch("sajucandle.api.routers.saju._has_anthropic_key", return_value=False):
            resp = client.post("/api/saju/report", json={
                "year": 1985, "month": 3, "day": 15,
                "gender": "M",
            })
